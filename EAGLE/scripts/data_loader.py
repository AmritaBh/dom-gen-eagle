...
def load_texts(data_file, label=False, expected_size=None):
    ...
    for line in tqdm(open(data_file), desc=f'Loading {data_file}'):
        ...
        domain_labels.append(json.loads(line)['domain_label'])
    ...
    if label:
        ...
        return texts, texts_perturb, label, domain_labels

    return texts, texts_perturb, domain_labels
...
class Corpus:
    def __init__(self, name, data_dir='data', label=False, skip_train=False, single_file=False):
        ...
        if single_file:
            ...
            if label:
                self.data, self.data_perturb, self.label, self.domain_label = load_texts(f'{data_dir}/{name}.jsonl', label=True)
            else:
                self.data = load_texts(f'{data_dir}/{name}.jsonl')

        else:

            self.train, self.train_perturb, self.train_domain_labels = load_texts(f'{data_dir}/{name}.train.jsonl') if not skip_train else None
            # self.test, self.test_perturb, self.test_domain_labels = load_texts(f'{data_dir}/{name}.test.jsonl') # no test file for source domains
            self.valid, self.valid_perturb, self.valid_domain_labels = load_texts(f'{data_dir}/{name}.valid.jsonl')


class EncodedDataset(Dataset):
    def __init__(self, real_texts: List[str], real_texts_perturb: List[str], real_dom_labels: List[str],
                 fake_texts: List[str], fake_texts_perturb: List[str], fake_dom_labels: List[str],
        ...
        self.real_dom_labels = real_dom_labels
        self.fake_dom_labels = fake_dom_labels
    ...
    def __getitem__(self, index):
        ...
        if index < len(self.real_texts):
            ...
            dom_label = self.real_dom_labels[index]
            label = 0
        else:
            text = self.fake_texts[index - len(self.real_texts)]
            text_perturb = self.fake_text_perturb[index - len(self.real_text_perturb)]
            dom_label = self.fake_dom_labels[index - len(self.real_text_perturb)]
            label = 1
        ...
        return torch.tensor(padded_sequences['input_ids']), torch.tensor(padded_sequences['attention_mask']), \
               torch.tensor(padded_sequences_perturb['input_ids']), torch.tensor(padded_sequences_perturb['attention_mask']), dom_label, label
